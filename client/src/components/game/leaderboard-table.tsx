          ...
          <tr>
            ...
            {!compact ? (
              <th className="px-4 py-3">
                {rankingType === "single" ? "Date" : "Win Ratio"}
              </th>
            ) : null}
          </tr>
        </thead>
        <tbody>
          {(rankingType === "single" ? singleRows : eloRows).map((row) => {
            const accuracy =
              rankingType === "single"
                ? Math.round(
                    ((row as LeaderboardRow).correctAnswers /
                      (row as LeaderboardRow).totalRounds) *
                      100,
                  )
                : null;
            const rankClass =
            ...
            return (
              <tr
                key={
                  rankingType === "single"
                    ? (row as LeaderboardRow).sessionId
                    : (row as EloLeaderboardRow).userId
                }
              ...
              >
                <td
                  className={
                    compact
                      ...
                      : "px-4 py-3 font-black text-[#cf1a4f]"
                  }
                >
                  {row.position}
                </td>
                <td
                  className={
                    compact ? "px-3 py-2 font-bold" : "px-4 py-3 font-bold"
                  }
                >
                  {row.nickname}
                </td>
                ...
                >
                  {rankingType === "single"
                    ? (row as LeaderboardRow).score
                    : (row as EloLeaderboardRow).elo}
                </td>
                <td className={compact ? "px-3 py-2" : "px-4 py-3"}>
                  {rankingType === "single"
                    ? `${accuracy}%`
                    : (row as EloLeaderboardRow).matchesPlayed}
                </td>
                {!compact ? (
